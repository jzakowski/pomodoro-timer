...
import { Plus, Trash2, Check, Star, Pencil, Save, X } from 'lucide-react'

export default function TasksTab() {
  const { tasks, addTask, deleteTask, editTask, toggleTaskComplete, setActiveTask } = useTasks()
  ...
  const saveEdit = (taskId: string) => {
    if (editTitle.trim()) {
      editTask(taskId, editTitle.trim())
                    ...
                    {editingTaskId === task.id ? (
                      <div className="flex-1 flex gap-2">
                        <input
                          ...
                          className="flex-1 px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-transparent outline-none text-lg font-semibold"
                          autoFocus
                        />
                        <button
                          onClick={() => saveEdit(task.id)}
                          className="px-3 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg transition-colors duration-150"
                        ...
                        <button
                          onClick={cancelEdit}
                          className="px-3 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg transition-colors duration-150"
